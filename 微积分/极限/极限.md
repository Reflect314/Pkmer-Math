...
数列极限是分析学的基础概念，通过ε-N语言严格定义收敛性，为后续研究[[级数收敛]]、[[连续性]]奠定基础。

![[数列极限]]

>[!example]- 经典示例
> 对任意 $\varepsilon > 0$，取 $N = \left\lfloor \frac{1}{\varepsilon} \right\rfloor + 1$，则当 $n > N$ 时：
> $$\left| \frac{1}{n} - 0 \right| = \frac{1}{n} < \frac{1}{N} = \frac{1}{\lfloor 1/\varepsilon \rfloor + 1} < \varepsilon$$
